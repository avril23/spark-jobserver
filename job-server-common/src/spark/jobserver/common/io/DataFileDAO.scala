package spark.jobserver.common.io
...
object DataFileDAO {
  ...
  val QUERY_INFO_FILE_NAME = "queryInfo.info"
...
class DataFileDAO(config: Config) {
  ...
  private def getWorkDir(uuid: String): File = {
    val basePath = config.getString("spark.jobserver.receiver.cache-folder")
    new File(basePath, uuid)
  }

  def readQueryInfo(uuid: String): String = {
    val queryInfoFile = new File(getWorkDir(uuid), DataFileDAO.QUERY_INFO_FILE_NAME)
    Source.fromFile(queryInfoFile).getLines().mkString
  }
