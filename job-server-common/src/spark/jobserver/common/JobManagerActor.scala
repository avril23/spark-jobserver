...
import spark.jobserver.{SparkJobValid, SparkJobInvalid, NamedObjectSupport, ContextLike}
import scala.concurrent.{Future, ExecutionContext}
import scala.util.{Failure, Success, Try}
import spark.jobserver.common.io.{JobDAOActor, JobInfo, JarInfo}
import spark.jobserver.common.utils.{ContextURLClassLoader, SparkJobUtils}
...
object JobManagerActor {
  ...
  case class CreateCache(tableName: String, filePath: String, format: String, columns: Map[String, String])

  case object Cached

  case class CacheError(t: Throwable)

  case class RetrieveCache(db: String, query: String)

  case class Retrieved(list: Option[(Map[String, String], Array[String])])

  case class RetrieveError(t: Throwable)
...
class JobManagerActor(contextConfig: Config) extends InstrumentedActor {
  ...
  def wrappedReceive: Receive = {
    ...
    case CreateCache(tableName: String, filePath: String, format: String, columns: Map[String, String]) =>
      try {
        val df = format match {
          case "json" =>
            val schema = StructType(columns.map(c => StructField(c._1, convertToCatalystType(c._2), nullable = true)).toList)
            sqlContext.read.schema(schema).json(filePath)
          case "parquet" =>
            val schema = StructType(columns.map(c => StructField(c._1, convertToCatalystType(c._2), nullable = true)).toList)
        ...
        case t: Throwable =>
          logger.error(s"create cache failed. TableName: $tableName, FilePath: $filePath", t)
          sender ! CacheError(t)
      }
  ...
  private def convertToCatalystType(typeStr: String): DataType = {
    typeStr match {
      case "int2" => ShortType
      case "int4" => IntegerType
      case "int8" | "bigserial" => LongType
      case "varchar" | "json" => StringType
  ...
  def createContextFromConfig(contextName: String = contextName): ContextLike = {
    ...
    val factory = factoryClass.newInstance.asInstanceOf[spark.jobserver.common.context.SparkContextFactory]
    Thread.currentThread.setContextClassLoader(jarLoader)
    val context = factory.makeContext(config, contextConfig, contextName)
    context
  }
